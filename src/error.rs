...
pub struct Error {
  ...
  #[schema(example = ErrorKind::InternalServer)]
  pub kind: ErrorKind,
}
...
impl From<sqlx::Error> for Error {
  fn from(error: sqlx::Error) -> Self {
    ...
    match error {
      sqlx::Error::Database(e) if e.kind() == SqlxErrorKind::UniqueViolation => {
        Self::new("data duplicate error", ErrorKind::ResourceConflict)
      }
      sqlx::Error::Database(e) if e.kind() == SqlxErrorKind::ForeignKeyViolation => {
        let message = match e.constraint() {
          Some(constraint) => format!("foreign key constraint violated: {}", constraint),
          None => "referenced resource does not exist".to_string(),
        };

        Self::new(message, ErrorKind::ForeignKeyViolation)
      }
...
impl<E, R> From<SdkError<E, R>> for Error {
  fn from(value: SdkError<E, R>) -> Self {
    Self::new(
      format!("storage service error: {}", value),
      ErrorKind::StorageService,
    )
  }
}
...
impl From<PresigningConfigError> for Error {
  fn from(value: PresigningConfigError) -> Self {
    Self::new(
      format!("error occured while generating presigned url: {}", value),
      ErrorKind::StorageService,
    )
  }
}
